...
def test_room_availability_available_when_no_reservations(auth_client, room):
    ...
    url = f"/pension/rooms/{room.id}/availability/?from_date=2026-02-10&to_date=2026-02-12&adults=2&children=0"
...
def test_room_unavailable_when_overlapping_blocking_reservation(auth_client, room, guest):
    ...
    url = f"/pension/rooms/{room.id}/availability/?from_date=2026-02-11&to_date=2026-02-13&adults=2&children=0"
    response = auth_client.get(url)
    assert response.status_code == 200
...
def test_room_available_when_dates_touch_at_boundary(auth_client, room, guest):
    ...
    url = f"/pension/rooms/{room.id}/availability/?from_date=2026-02-12&to_date=2026-02-14&adults=2&children=0"
    response = auth_client.get(url)
    assert response.status_code == 200
    assert response.data["available"] is True
...
def test_room_capacity_too_many_adults(auth_client, room):
    url = f"/pension/rooms/{room.id}/availability/?from_date=2026-02-10&to_date=2026-02-12&adults={room.max_adults+1}&children=0"
    response = auth_client.get(url)
    assert response.status_code == 200
    assert response.data["available"] is False
...
def test_room_capacity_too_many_children(auth_client, room):
    url = f"/pension/rooms/{room.id}/availability/?from_date=2026-02-10&to_date=2026-02-12&adults=1&children={room.max_children+1}"
    response = auth_client.get(url)
    assert response.status_code == 200
    assert response.data["available"] is False
...
def test_room_capacity_too_many_total(auth_client, room):
    ...
    url = f"/pension/rooms/{room.id}/availability/?from_date=2026-02-10&to_date=2026-02-12&adults={room.capacity}&children=1"
    response = auth_client.get(url)
    assert response.status_code == 200
    assert response.data["available"] is False
...
def test_room_availability_missing_from_date_or_to_date(auth_client, room):
    url = f"/pension/rooms/{room.id}/availability/?to_date=2026-02-12&adults=1&children=0"
    response = auth_client.get(url)
    assert response.status_code == 400
    url2 = f"/pension/rooms/{room.id}/availability/?from_date=2026-02-10&adults=1&children=0"
...
def test_room_availability_invalid_date_range(auth_client, room):
    ...
    url = f"/pension/rooms/{room.id}/availability/?from_date=2026-02-12&to_date=2026-02-10&adults=1&children=0"
    response = auth_client.get(url)
    assert response.status_code == 400
    url2 = f"/pension/rooms/{room.id}/availability/?from_date=2026-02-10&to_date=2026-02-10&adults=1&children=0"
    response2 = auth_client.get(url2)
    assert response2.status_code == 400